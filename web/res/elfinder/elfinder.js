	...
	this.not('.elfinder-cwd').each(function() {
		...
		var 
			...
			content = function(files, any) {
				...
				list && cwd.html('<table><thead class="header"><tr class="ui-state-default"><td >'+msg.name+'</td><td>'+msg.perm+'</td><td>'+msg.mod+'</td><td>'+msg.size+'</td><td>'+msg.kind+'</td></tr></thead><tbody/></table>');
...
$.fn.elfinderdialog = function(opts) {
	...
	this.filter(':not(.ui-dialog-content)').each(function() {
		var self       = $(this).addClass('ui-dialog-content ui-widget-content mod-body'),
			parent     = self.parent(),
			...
			buttonset  = $('<div class="ui-dialog-buttonset buttons"/>'),
			buttonpane = $('<div class=" ui-helper-clearfix ui-dialog-buttonpane mod-act ui-widget-content"/>')
				.append(buttonset),
			
			dialog = $('<div class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable std42-dialog box modal show '+cldialog+' '+opts.cssClass+'"/>')
				.hide()
				.append(self)
				.appendTo(parent)
		...
		dialog.prepend(
			$('<div class="ui-dialog-titlebar ui-widget-header header ui-helper-clearfix">'+opts.title+'</div>')
				.prepend($('<a href="#" class="ui-dialog-titlebar-close ui-corner-all"><span class="ui-icon ui-icon-closethick icon icon-remove"/></a>')
					.mousedown(function(e) {
						e.preventDefault();
						self.elfinderdialog('close');
					}))
...
$.fn.elfindersearchbutton = function(cmd) {
	return this.each(function() {
		...
		$('<span class="ui-icon ui-icon-search icon icon-search" title="'+cmd.title+'"/>')
			.appendTo(button)
			.click(search);
		
		$('<span class="ui-icon ui-icon-close icon icon-remove"/>')
			.appendTo(button)
			.click(abort)
...
$.fn.elfindersortbutton = function(cmd) {
	...
	return this.each(function() {
		...
		$.each(fm.sortRules, function(name, value) {
			menu.append($('<div class="'+item+'" rel="'+name+'"><span class="ui-icon ui-icon-arrowthick-1-n arrow">&#39;</span><span class="ui-icon ui-icon-arrowthick-1-s arrow">&amp;</span>'+fm.i18n('sort'+name)+'</div>').data('type', name));
		});
		...
		$('<div class="'+item+' '+item+'-separated"><span class="ui-icon ui-icon-check icon icon-ok"/>'+fm.i18n('sortFoldersFirst')+'</div>')
			.appendTo(menu)
			.click(function() {
...
$.fn.elfindertoolbar = function(fm, opts) {
	this.not('.elfinder-toolbar').each(function() {
		var commands = fm._commands,
			self     = $(this).addClass('ui-helper-clearfix elfinder-toolbar header'),
			panels   = opts || [],
			l        = panels.length,
			i, cmd, panel, button;
...
elFinder.prototype.commands.quicklook = function() {
	var self       = this,
		...
		navbar  = $('<div class="elfinder-quicklook-navbar mod-body"/>')
			.append($('<div class="'+navicon+' '+navicon+'-prev arrow">%</div>').mousedown(function() { navtrigger(37); }))
			.append(fsicon)
			.append($('<div class="'+navicon+' '+navicon+'-next arrow">(</div>').mousedown(function() { navtrigger(39); }))
			// .append($('<div class="'+navicon+' '+navicon+'-close"/>').mousedown(function() { self.window.trigger('close'); }))
		;

	this.resize = 'resize.'+fm.namespace;
	this.info = $('<div class="elfinder-quicklook-info-wrapper mod-body"/>')
		.append(icon)
		.append(info);
		
	this.preview = $('<div class="elfinder-quicklook-preview ui-helper-clearfix mod-body"/>')
		// clean info/icon
	...
	this.window = $('<div class="ui-helper-reset ui-widget elfinder-quicklook box modal mini show" style="position:absolute"/>')
		.click(function(e) { e.stopPropagation();  })
		.append(
			$('<div class="elfinder-quicklook-titlebar header"/>')
				.append(title)
				.append($('<button class="ui-icon close mini"><span class="icon icon-remove no-mg"></span></button>').mousedown(function(e) {
					e.stopPropagation();
	...
	this.init = function() {
		...
		fm.one('load', function() {
			...
			win.appendTo('#content').zIndex(100 + parent.zIndex());
		...
		var fm    = this.fm,
			...
			open = function(file, id) {
				var dialog   = $('<div class="elfinder-dialog-resize"/>'),
					...
					uibuttonset = '<div class="ui-widget-content ui-corner-all elfinder-buttonset buttons resize-btn-group"/>',
					...
					uiprop   = $('<span />'),
					reset    = $('<div class="ui-state-default button elfinder-resize-reset"><span class="icon icon-refresh"/></div>'),
					//uitype   = $('<div class="elfinder-resize-type"><div class="elfinder-resize-label">'+fm.i18n('mode')+'</div></div>')
					uitype   = $('<div class="elfinder-resize-type buttons btn-radio"/>')
					...
					constr  = $('<div class="checkbox-cont"><input type="checkbox" checked="checked" style="display: none;"/><div class="on grad2"></div></div>')
						.change(function() {
